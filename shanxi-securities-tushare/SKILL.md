...
本 skill 只负责**取数与数据交付**；若用户要求对标的进行分析/评估/对比/深度研究，应使用 `sxsc-tushare-analysis` skill。

- 获取单只股票/指数/ETF 的行情、估值、活跃度数据
- 多标的数据的批量取数（业绩、估值、涨幅、ROE 等字段）
- 获取财务数据（利润趋势、ROE、毛利率、现金流）
- 获取资金流数据（北向、主力、龙虎榜、板块吸金）
- 获取板块轮动、指数成分股数据
- 获取宏观数据（CPI / PPI / GDP / PMI / 利率 / 货币供应量）
...
| 任务类型 | 典型表达 |
|---|---|
| 行情/趋势数据 | 取最近行情、涨跌、K线、量能数据 |
| 基本资料 | 什么公司、什么时候上市、行业 |
| 财务数据 | 拉财报、利润、ROE、现金流 |
| 估值数据 | 取 PE/PB、股息率数据 |
| 资金流/市场行为数据 | 取北向、主力、龙虎榜、板块吸金数据 |
| 板块/指数/主题数据 | 取板块成分股、指数成分、概念数据 |
| 涨跌停/情绪数据 | 取涨停梯队、连板、活跃度数据 |
| 公告/新闻/研报 | 公告、催化、新闻面（**受限**：仅告知边界 + 替代路径） |
| 宏观/跨市场 | CPI、PMI、利率、港股、美股 |
| 数据导出/研究准备 | 拉 CSV、回测数据表、Parquet |

> 若用户要求对标的进行**分析/评估/对比/深度研究**，应切换至 `sxsc-tushare-analysis` skill。
