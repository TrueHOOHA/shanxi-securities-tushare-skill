...
| 2 | 行情趋势 | `daily`、`weekly`、`monthly`、`daily_basic`、**`adj_factor`**、`index_daily`(沪深300+行业指数) | 近 20/60/250 日涨跌幅（基于复权价）、MA5/MA20/MA60、换手率、振幅、波动率、阶段最高/最低；**跑赢/跑输沪深300及所属申万行业指数** |
| 3 | 估值分析 | `daily_basic`、`index_dailybasic`(行业) | PE(TTM)、PB、PS(TTM)、股息率、总市值、流通市值；**与行业均值对比**（取 `index_classify` 获取行业指数代码 → `index_dailybasic` 取行业 PE/PB 均值） |
...
- **ST/*ST 股票**：涨跌停幅度为 5%（非普通股的 10%），`stk_limit` 返回的涨停价会体现这一差异；分析涨跌停维度时需区分 ST 与非 ST。
